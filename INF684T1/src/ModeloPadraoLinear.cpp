...
ModeloPadraoLinear::ModeloPadraoLinear(Dados& d) :Base(d) {
    ;
}
void ModeloPadraoLinear::rodar() {
    IloEnv env;
    ...
    IloNumVarArray u(env, d->n_locais, 0.0, IloInfinity, ILOFLOAT);
     ...
     BoolVar3Matrix l(env, d->n_locais);
     for (int i = 0; i < d->n_locais; i++) {
         l[i] = BoolVarMatrix(env, d->n_locais);
         for (int j = 0; j < d->n_locais; j++) {
             l[i][j] = IloBoolVarArray(env, d->n_locais);
             for (int k = 0; k < d->n_locais; k++) {
                 l[i][j][k] = IloBoolVar(env);
             }
         }
     }
    ...
    for (int i = 0; i < d->n_locais; i++) {
        for (int j = 1; j < d->n_locais; j++) {
            ...
            for (int k = 0; k < d->n_locais; k++) {
                ...
                mod.add(l[i][j][k] <=X[i][j]);
                mod.add(l[i][j][k] <= X[j][k]);
                mod.add(l[i][j][k] >= X[i][j]+ X[j][k] -1);
